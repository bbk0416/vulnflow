...
## 72.0.98 Greenbone multi-CVE CVSS attribution

72.0.98은 Greenbone NVT가 여러 CVE를 참조할 때 하나의 NVT-level CVSS를 모든 expanded CVE에 복제하지 않습니다. GMP XML은 `epss/max_severity`와 `epss/max_epss`의 nested `cve@id`와 `<severity>`가 명시하는 대표 CVE에만 해당 CVSS를 귀속하고, 대표되지 않은 CVE에는 다른 CVE의 점수를 추측해 복제하지 않습니다. detailed CSV는 다중-CVE 행에서 대표 CVE ID를 제공하지 않으므로 per-CVE CVSS를 비우고 parser warning을 남깁니다. 단일-CVE CVSS와 72.0.97 result-path identity, 72.0.96 CSV EPSS fail-safe, 72.0.95 XML per-CVE EPSS 귀속, schema 46과 dependency package pins는 변경하지 않습니다.


## 72.0.97 Greenbone XML result path identity

72.0.97은 Greenbone GMP XML result의 non-empty `<path>`를 canonical component identity와 notes에 보존합니다. 72.0.96까지는 동일 자산·CVE·NVT·host-level port에 서로 다른 로컬 파일 경로가 있는 별도 scanner result가 같은 component와 generated finding identity로 충돌해 하나가 duplicate로 거부될 수 있었습니다. 72.0.97은 서로 다른 result path를 별도 finding으로 유지하며, path가 없는 XML과 기존 numeric port identity, 72.0.96의 CSV multi-CVE EPSS fail-safe, 72.0.95의 XML per-CVE EPSS 귀속, schema 46과 dependency package pins는 변경하지 않습니다.
...
72.0.101은 Greenbone 공식 detailed CSV의 `Affected software/operating system` 값을 component identity와 notes에 보존해 동일 host/CVE/VT/port에서 서로 다른 affected software 결과가 동일 automatic finding ID로 충돌하거나 import 전체를 중복 ID 오류로 중단시키지 않게 합니다. 72.0.100 OCI image identity, 72.0.99 Nessus multi-CVE CVSS fail-safe, 72.0.98 Greenbone multi-CVE CVSS 귀속, 72.0.97 result-path identity, schema 46과 dependency package pins는 변경하지 않습니다.
