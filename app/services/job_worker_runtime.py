...
def _worker_project_selections(context: ApplicationContext) -> list[ProjectSelection | None]:
    return application_project_selections(context)
        ...
        try:
            ...
            if bind_operation_guard(context).restore_in_progress():
                if await wait_interval():
                    return
                continue
            selections = _worker_project_selections(context)
            if selections:
                start = project_cursor % len(selections)
                selections = selections[start:] + selections[:start]
                project_cursor = (start + 1) % len(selections)
            job: dict[str, Any] | None = None
            selected: ProjectSelection | None = None
            for candidate in selections:
                if project_is_read_only(context, candidate):
                    continue
                scope = project_scope(candidate) if candidate is not None else nullcontext()
                with scope:
                    if bind_operation_guard(context).restore_in_progress():
                        continue
                ...
                try:
                    ...
                    context.logger.exception(
                        ...
                        extra={
                            ...
                            "project_id": selected.project_id if selected else "default",
                        },
                    )
                    try:
