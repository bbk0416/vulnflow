...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="vulnflow_reconciliation_") as temp_dir:
        ...
        os.environ["VULNFLOW_API_TOKENS_JSON"] = json.dumps({
            "scanner-ci": {"token": "reconciliation-operator-token-12345", "role": "operator"}
        })
        ...
        with TestClient(app_main.app) as client:
            ...
            csrf = client.get("/", auth=admin).cookies.get(app_main.CSRF_COOKIE) or client.cookies.get(app_main.CSRF_COOKIE)
            a = (
            ...
            for source, payload in (("scanner-a", a), ("scanner-b", b)):
                response = client.post(
                    ...
                    follow_redirects=False, auth=admin,
                )
                assert response.status_code == 303, (source, response.status_code, response.text)

            detail = client.get("/api/v1/findings/A-20/sources", auth=admin)
            ...
            finding = client.get("/api/v1/findings/A-20", auth=admin).json()
            ...
            assert client.get("/api/v1/findings/A-20", auth=admin).json()["record_state"] == "ACTIVE"
            ...
            assert client.get("/api/v1/findings/A-20", auth=admin).json()["record_state"] == "STALE"

        report = ROOT / "reports" / "reconciliation_verification.txt"
        report.parent.mkdir(parents=True, exist_ok=True)
        report.write_text(
            "VulnFlow 72.0.12 multi-scanner reconciliation smoke\n"
