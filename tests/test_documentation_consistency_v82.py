...
def test_stale_public_regression_count_fails_closed(tmp_path: Path) -> None:
    ...
    path = root / "README.md"
    path.write_text(path.read_text(encoding="utf-8").replace("**721개**", "**720개**"), encoding="utf-8")
    ...
    verification.write_text(
        verification.read_text(encoding="utf-8").replace(
            "public manifest: 689/689 PASS",
            "public manifest: 678/678 PASS",
        ),
        encoding="utf-8",
    )
    ...
    verification.write_text(
        verification.read_text(encoding="utf-8").replace(
            "release notes: RELEASE_NOTES_72.0.96.md",
