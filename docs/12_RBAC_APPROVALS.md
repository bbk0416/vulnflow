...
브라우저 사용자는 제어 SQLite DB의 `app_users` 테이블에서 관리하며 비밀번호는 scrypt 해시로만 저장합니다. 최초 관리자는 `python -m scripts.manage_users --db ./data/control.db create --username admin --role admin`으로 생성합니다. 로그인 후에는 원문을 저장하지 않는 불투명 세션 쿠키를 사용합니다. 실패 로그인은 기본 300초 sliding window에서 username+client 기준 5회, client 전체 기준 25회로 제한하며 **계정 전역 잠금은 사용하지 않습니다**. 비활성화 시 전체 세션 종료, 비밀번호 변경 시 기존 세션 폐기를 적용합니다. 관리자 화면 `/admin/users`에서 계정 생성·활성화·비활성화·비밀번호 초기화·세션 종료를 수행합니다.
