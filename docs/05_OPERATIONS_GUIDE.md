...
애플리케이션은 사용자·프로젝트 등록정보용 `data/control.db`와 기본 프로젝트 데이터용 `data/projects/default/vulnflow.db`를 분리해 사용하며 운영 모드에서는 빈 데이터베이스로 시작합니다. `VULNFLOW_DB`의 `data/vulnflow.db`는 72.0.24 이하 단일 DB 업그레이드 호환 경로입니다. 합성 샘플은 명시적인 데모 모드에서만 적재됩니다. 최초 실행 전에 관리자 계정과 프로젝트별 저장 경로를 확인합니다.
